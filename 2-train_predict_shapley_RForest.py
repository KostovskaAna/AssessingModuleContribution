...
def train_predict(conf, modAlgo):
    ...
    for dim in [5, 30]:
        for budget in [50*dim, 100*dim, 300*dim, 500*dim, 1000*dim, 1500*dim]: 
            ...
            for train_index_outer, test_index_outer in logo_outer.split(X, y, groups_outer):
                ...
                shapley_folder = f'/home/akostovska/GECCO_journal_extension/results_re/{modAlgo}/shapley'
                if not os.path.exists(shapley_folder):
                    os.makedirs(shapley_folder)
                df_shap_train.to_csv(f'/home/akostovska/GECCO_journal_extension/results_re/{modAlgo}/shapley/shapley_budget_{budget}_conf_{conf}_{dim}D_fold_{fold}_train.csv')
                df_shap_test.to_csv(f'/home/akostovska/GECCO_journal_extension/results_re/{modAlgo}/shapley/shapley_budget_{budget}_conf_{conf}_{dim}D_fold_{fold}_test.csv')
                # save best model in each inner cv fold
                models_folder = f'/home/akostovska/GECCO_journal_extension/results_re/{modAlgo}/regression_models'
                if not os.path.exists(models_folder):
                    os.makedirs(models_folder)
                joblib.dump(best_inner_model, f'/home/akostovska/GECCO_journal_extension/results_re/{modAlgo}/regression_models/budget_{budget}_conf_{conf}_{dim}D_test_fold_{fold}.pkl', compress = 1)
            # save predictions on the outer test set
            predictions_folder = f'/home/akostovska/GECCO_journal_extension/results_re/{modAlgo}/regression_predictions'
            if not os.path.exists(predictions_folder):
                os.makedirs(predictions_folder)
            test_preds_df = pd.DataFrame(test_preds, columns=["predicted"])
            test_preds_df.index = test_preds_index
            test_preds_df.to_csv(f'/home/akostovska/GECCO_journal_extension/results_re/{modAlgo}/regression_predictions/budget_{budget}_conf_{conf}_{dim}D_test_fold.csv')
    ...
    performance_df.to_csv(f'/home/akostovska/GECCO_journal_extension/results_re/{modAlgo}/regression_performance_tables/RForest_conf_{conf}.csv')
